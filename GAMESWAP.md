...
├── Start-GameSwap.bat        Lanceur (élève les droits admin automatiquement)
├── Uninstall.ps1             Désinstallation complète
├── Uninstall.bat             Lanceur de désinstallation (élève les droits)
├── GAMESWAP.md               Cette documentation
│
├── Modules/
...
Start-GameSwap.bat
  └── Élève les droits admin
      └── GameSwap.ps1
            ...
            [Lancement normal]
            6a. Vérifie que le partage SMB "GameSwap" existe → le recrée si absent
            6b. Set-GSQueueMaxSlots → s'assure que maxSlots est publié
            6c. Install-NmapIfNeeded
...
| `Dismount-GSVhdx` | Démonte le VHDX actuellement suivi par la variable `$script:MountedVhdxPath` |
...
| `Get-GSGameExtraInfo -XmlPath` | Lit `[GameName]_info.xml` (Trailer, MaxPlayers, Thumbnail, Description, Instructions) |
...
| Onglet | Contenu |
|---|---|
| **Mes Jeux** | Liste des jeux locaux (DataGrid), boutons Installer/Jouer/Serveur/Désinstaller/Supprimer, panneau de détails à droite (vignette, joueurs max, trailer, description, instructions) |
| **Réseau** | Scan réseau, liste des joueurs, liste des jeux disponibles, téléchargement avec barre de progression, file d'attente automatique, panneau de détails à droite |
| **Paramétrage** | Nom du joueur, dossier GameSwap (avec déplacement), nombre de téléchargements simultanés, liste des téléchargements actifs avec libération manuelle |
| **Logs** | Affichage des logs en temps réel (optionnel) |
