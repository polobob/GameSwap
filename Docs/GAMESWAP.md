...
| `Get-RemoteGameList -HostIP` | Connexion au partage distant, liste les dossiers `Jeux/`, retourne les objets jeux avec métadonnées. Lit les bytes de la vignette (`ThumbBytes [byte[]]`) pendant que le partage est encore connecté |
...
| `Copy-RemoteGame -RemoteVhdxPath -LocalGamesFolder -GameName -HostIP -PlayerName -ShareRootPath -OnProgress -OnComplete -OnError` | Téléchargement dans un Runspace séparé (non bloquant). Copie aussi `_info.xml` et vignette. Libère le slot dans le `finally`. Le SyncHash contient `GameName` et `HostIP` (accessibles depuis le DispatcherTimer après la fin du scriptblock) |
...
Lancer `Tools\Uninstall.bat` (droits admin requis). Les étapes :
...
4. Désinstallation silencieuse de nmap : recherche de l'entrée nmap dans le registre (`HKLM:\SOFTWARE\...\Uninstall`), extraction de l'exécutable de désinstallation, exécution avec le flag NSIS `/S`
...
### Désinstallation
- `Tools\Uninstall.ps1` supprime le partage SMB, le compte Windows, les settings AppData et nmap.
- Propose séparément de supprimer le dossier des jeux (avec démontage préalable des VHDX).
- nmap est désinstallé silencieusement via son NSIS uninstaller trouvé dans le registre (flag `/S`), et non via `winget` qui ne transmet pas toujours le flag silencieux aux installeurs NSIS.
...
### Réorganisation de la racine du projet
- `GAMESWAP.md` déplacé dans `Docs/` (documentation technique, pas utile pour l'utilisateur final).
- `Uninstall.bat` et `Uninstall.ps1` déplacés dans `Tools/` (usage rare, hors du chemin normal d'utilisation).
- La racine expose uniquement les fichiers qu'un utilisateur doit voir : `Start-GameSwap.bat`, `GameSwap_Guide.html`, `README.md`.

### Vignettes distantes — chargement en mémoire
- Problème : après un téléchargement + installation + retour à l'onglet Réseau, les vignettes des jeux distants disparaissaient. Cause : `Get-RemoteGameList` déconnecte le partage SMB dans son bloc `finally` ; le `ThumbPath` UNC stocké dans les objets devenait inaccessible une fois la session SMB expirée du cache Windows.
- Solution : `Get-RemoteGameList` lit les bytes de l'image avec `[System.IO.File]::ReadAllBytes()` pendant que le partage est encore connecté et les stocke dans `ThumbBytes ([byte[]])`. Dans l'UI, le `BitmapImage` est reconstruit depuis un `MemoryStream` — aucun accès réseau requis après le chargement initial.

### DispatcherTimer — portée des variables locales
- Problème : les variables `$gameName` et `$hostIP` définies dans `$script:StartDownload` étaient vides dans le `Add_Tick` du DispatcherTimer. Cause : quand le timer se déclenche, le scope local du scriptblock appelant est déjà terminé.
- Solution : stocker `GameName` et `HostIP` dans le `SyncHash` de `Copy-RemoteGame` (accessible via `$script:DlHandle.SyncHash` depuis n'importe quel scope).
- Règle générale : toute variable nécessaire dans un `DispatcherTimer.Add_Tick` doit être dans un `$script:` ou dans un objet partagé (SyncHash), jamais dans une variable locale d'un scriptblock parent.

### Désinstallation nmap silencieuse
- `winget uninstall --silent` ne transmet pas toujours le flag silencieux aux installeurs NSIS (comme nmap).
- Solution : rechercher l'entrée de désinstallation dans le registre (`HKLM:\SOFTWARE\...\Uninstall`), extraire l'exécutable depuis `UninstallString`, l'exécuter avec `/S` (flag NSIS universel pour désinstallation silencieuse).
